...
unsafe fn execute_with_syscall_support(entry_point: usize) -> usize {
    ...
    //console_println!("[i] Setting up REAL user mode execution with REAL syscalls!");
    ...
    console_println!("[i] Exit stub instructions:");
    ...
    console_println!("[i] User mode context set up:");
    console_println!("   Entry point: 0x{:x}", entry_point);
    ...
    console_println!("[o] Returned from user mode. {}", result);
    result
}
...
unsafe fn execute_user_program_virtual(entry_point: usize) {
    ...
    console_println!("[i] About to execute at virtual address 0x{:x}", entry_point);
    
    console_println!("[i] User space trap handling already set up by main trap handler");
    
    console_println!("[i] Switching to user address space...");
    if let Err(e) = crate::memory::mmu::switch_to_user_space() {
        console_println!("[x] Failed to switch to user space: {}", e);
        return;
    }
    console_println!("[o] Switched to user address space successfully!");
    
    // Set up user stack
    console_println!("[i] Setting up user stack...");
    ...
    if let Err(e) = crate::memory::mmu::map_elf_segment(
        ...
        console_println!("[x] Failed to map user stack: {}", e);
        return;
    }
    
    let stack_top = stack_vaddr + stack_size - 8; // Leave some space at top
    console_println!("[o] User stack mapped at 0x{:x} - 0x{:x}, SP will be 0x{:x}", 
        stack_vaddr, stack_vaddr + stack_size, stack_top);
    
    console_println!("[i] Executing user program...");
...
unsafe fn execute_user_program(entry_point: usize) {
    ...
    if let Some(stack_addr) = crate::memory::allocate_memory(4096) {
        ...
        console_println!("[i] Setting up user program wrapper...");
